...
const SidebarNavLink = ({ to, icon, text, disabled = false }: NavItemProps) => {
    if (disabled) {
        return (
            <div className="flex items-center p-2 text-foreground-muted-light dark:text-foreground-muted-dark rounded-lg cursor-not-allowed group" title="Coming Soon">
                {icon}
...
const MobileNavLink = ({ to, icon, text, disabled = false }: NavItemProps) => {
    if (disabled) {
        return (
            <div className="inline-flex flex-col items-center justify-center px-5 text-foreground-muted-light dark:text-foreground-muted-dark cursor-not-allowed" title="Coming Soon">
                {icon}
    ...
    return (
        <NavLink to={to} className={({ isActive }) =>
            `inline-flex flex-col items-center justify-center px-5 hover:bg-secondary-light dark:hover:bg-secondary-dark group transition-colors ${isActive ? 'text-blue-600 dark:text-blue-500' : 'text-foreground-muted-light dark:text-foreground-muted-dark'}`
        }>
            {icon}
            <span className="text-sm">{text}</span>
        </NavLink>
            ...
            <aside className="fixed top-0 left-0 z-40 w-64 h-screen transition-transform -translate-x-full sm:translate-x-0">
                <div className="h-full px-3 py-4 overflow-y-auto bg-primary-light dark:bg-primary-dark flex flex-col">
                    ...
                    <div className="mt-auto pt-4 space-y-2 font-medium border-t border-secondary-light dark:border-secondary-dark">
                         <a href="github://" className="flex items-center p-2 text-foreground-light rounded-lg dark:text-foreground-dark hover:bg-secondary-light dark:hover:bg-secondary-dark group transition-colors">
                            <GithubIcon />
                            <span className="ms-3">GitHub</span>
                        </a>
                        <a href="https://dash.cloudflare.com" target="_blank" rel="noopener noreferrer" className="flex items-center p-2 text-foreground-light rounded-lg dark:text-foreground-dark hover:bg-secondary-light dark:hover:bg-secondary-dark group transition-colors">
                            <CloudflareIcon />
